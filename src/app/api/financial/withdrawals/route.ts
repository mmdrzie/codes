  ...
  try {
    ...
    if (result.success) {
      ...
      const response: WithdrawalResponse = {
        ...
        balanceAfterWithdrawal: balance?.currentBalance
      };
...
export async function GET(request: NextRequest) {
  try {
    ...
    // In a real system, this would fetch withdrawal records for the account
    return Response.json({
      success: true,
      withdrawals: [],
      count: 0,
    ...
    logger.error('Withdrawal records retrieval error:', error);
